...
const CancelledBuyerTable: React.FC<Props>  = ({setSelectedOrder}) => {
    const [currentPage, setCurrentPage] = useState(1);
    const [pageSize, setPageSize] = useState(6);

    const data = allorderData.filter((item)=> item.status === 'Cancelled')
    ...
    return (
        <MyCard className="text-white justify-between">
            <Heading type="pageHeading" title="Cancelled Orders" />
            <MyTable<orderlist>
                columns={columns}
